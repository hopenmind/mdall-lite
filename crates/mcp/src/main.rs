...
fn tool_defs() -> Value {
    json!([
        ...
        {
            "name": "equation_history",
            "description": "List the timestamped edit history of every equation in a document (the local 'git for equations' log). Returns each equation's id, origin LaTeX, current LaTeX, change count, and the full dated change list (ts, by, from, to, reason).",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "input": { "type": "string", "description": "Absolute path to the document (its history is keyed by this path)." }
                },
                "required": ["input"],
                "additionalProperties": false
            }
        ...
        {
            "name": "equation_revert",
            "description": "Revert one equation to a prior version. 'to' is either 'origin' or the exact timestamp (ts) of a recorded change. The revert is appended to the history (never rewrites it); for a .md file the document is rewritten so the equation carries the reverted LaTeX.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "input": { "type": "string", "description": "Absolute path to the document (.md to also rewrite the source)." },
                    "id":    { "type": "string", "description": "The equation id (from equation_history)." },
                    "to":    { "type": "string", "description": "'origin' or the ts of the target change." }
                },
                "required": ["input", "id", "to"],
                "additionalProperties": false
            }
        },
        {
            "name": "equation_lint",
            "description": "Check every display equation for basic malformation (unbalanced braces, mismatched \\left/\\right). Read-only. Provide either 'input' (a file path) or 'markdown'. Returns the problem list.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "input":    { "type": "string", "description": "Absolute path to a document to read first." },
        ...
        {
            "name": "equation_fix",
            "description": "Auto-correct malformed display equations in a .md file: normalize LaTeX escapes and balance braces / \\left..\\right. Each fix is recorded in the history (by='mcp'), so it can be reverted. Injects equation id markers if absent. Pass 'id' to fix one equation, else all malformed ones are fixed.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "input": { "type": "string", "description": "Absolute path to the .md file to fix in place." },
                    "id":    { "type": "string", "description": "Optional: fix only the equation with this id." }
                },
                "required": ["input"],
                "additionalProperties": false
            }
...
fn handle_call(params: Option<&Value>, id: Value) -> Value {
    ...
    let result: Result<Vec<Value>, String> = match name {
        ...
        "equation_history" => call_equation_history(&args),
        "equation_revert" => call_equation_revert(&args),
        "equation_lint" => call_equation_lint(&args),
        "equation_fix" => call_equation_fix(&args),
...
/// Shared history home for both apps: %APPDATA%/MD-ALL/history/ (document-centric).
const HISTORY_APP: &str = "MD-ALL";

fn call_equation_history(args: &Value) -> Result<Vec<Value>, String> {
    let input = str_arg(args, "input")?;
    let h = mdall_core::equation_history::history_for(HISTORY_APP, input);
    let equations: Vec<Value> = h
        .equations
        .iter()
        .map(|eq| {
            json!({
                ...
                "origin": eq.origin,
                "current": eq.current(),
                "change_count": eq.change_count(),
                "changes": eq.changes.iter().map(|c| json!({
                    "ts": c.ts, "by": c.by, "from": c.from, "to": c.to, "reason": c.reason
                })).collect::<Vec<_>>(),
            })
        })
        .collect();
    let v = json!({
        "input": input,
        "doc_id": h.doc_id,
        "initial_file": h.initial_file,
        "created": h.created,
        "equation_count": h.equations.len(),
        "equations": equations,
    });
    Ok(vec![text_block(&pretty(&v))])
}

fn call_equation_revert(args: &Value) -> Result<Vec<Value>, String> {
    let input = str_arg(args, "input")?;
    let id = str_arg(args, "id")?;
    let to = str_arg(args, "to")?;
    let target = mdall_core::equation_history::revert(HISTORY_APP, input, id, to, "mcp")
        .ok_or_else(|| format!("no history for equation '{}' in {}", id, input))?;
    let rewrote = if input.to_ascii_lowercase().ends_with(".md") {
        let md = std::fs::read_to_string(input).map_err(|e| format!("cannot read {}: {}", input, e))?;
        match mdall_core::equation_history::set_equation_latex(&md, id, &target) {
            Some(new_md) => {
                std::fs::write(input, new_md).map_err(|e| format!("cannot write {}: {}", input, e))?;
                true
            }
            None => false,
        }
    ...
    let v = json!({
        ...
        "reverted_to": to,
        "latex": target,
        "document_rewritten": rewrote,
    });
    Ok(vec![text_block(&pretty(&v))])
}

fn call_equation_lint(args: &Value) -> Result<Vec<Value>, String> {
    let md = resolve_markdown(args)?;
    let ids = mdall_core::equation_history::scan_ids(&md);
    let mut problems = Vec::new();
    for (i, (id, latex)) in ids.iter().enumerate() {
        if let Some(problem) = mdall_core::equation_history::latex_problem(latex) {
            problems.push(json!({
                "ordinal": i + 1,
...
/// Best-effort repair: normalize escapes, then balance `{}` and `\left`/`\right`.
fn fix_latex(latex: &str) -> String {
    let mut s = mdall_core::render::normalize_latex_escapes(latex);
    let depth: i32 = s.chars().fold(0, |d, c| match c {
        '{' => d + 1,
        '}' => d - 1,
        _ => d,
    });
    ...
    for _ in 0..lefts.saturating_sub(rights) {
        s.push_str(" \\right.");
    }
    s
}

fn call_equation_fix(args: &Value) -> Result<Vec<Value>, String> {
    let input = str_arg(args, "input")?;
    if !input.to_ascii_lowercase().ends_with(".md") {
        return Err("equation_fix requires a .md file (it rewrites the source in place)".to_string());
    }
    let only_id = args.get("id").and_then(Value::as_str);
    let raw = std::fs::read_to_string(input).map_err(|e| format!("cannot read {}: {}", input, e))?;
    let (mut md, ids) = mdall_core::equation_history::ensure_ids(&raw);
    ...
    for (id, latex) in &ids {
        ...
        if mdall_core::equation_history::latex_problem(latex).is_none() {
            continue;
        }
        let candidate = fix_latex(latex);
        if candidate != *latex && mdall_core::equation_history::latex_problem(&candidate).is_none() {
            if let Some(new_md) = mdall_core::equation_history::set_equation_latex(&md, id, &candidate) {
                md = new_md;
                mdall_core::equation_history::record_change(
                    HISTORY_APP, input, id, latex, &candidate, "mcp", Some("auto-fix (lint)"),
                );
                fixed.push(json!({ "id": id, "from": latex, "to": candidate }));
            }
        }
    }
    std::fs::write(input, &md).map_err(|e| format!("cannot write {}: {}", input, e))?;
    let v = json!({ "input": input, "fixed_count": fixed.len(), "fixed": fixed });
    Ok(vec![text_block(&pretty(&v))])
}
