...
impl MdApp {
    ...
    fn apply_equation_edit(&mut self) {
        ...
        if let Some(range) = range_opt {
            // Resolve this equation's history id (by its ordinal among $$ blocks)
            // BEFORE mutating the source.
            let hist_ord = mdall_core::equation_history::equation_index_at(&self.source, range.start);
            ...
            // Log the change to the edit history (skip silently if untracked).
            if let (Some(ord), Some(file)) = (hist_ord, self.current_file.clone()) {
                if let Some((id, old)) = self.doc_eq_ids.get(ord).cloned() {
                    let path = file.to_string_lossy().into_owned();
                    mdall_core::equation_history::record_change(
                        "MD-ALL", &path, &id, &old, &new_latex, "user", None,
                    );
                    if let Some(slot) = self.doc_eq_ids.get_mut(ord) {
                        slot.1 = new_latex.clone();
                    }
                }
            }
        } else {
