...
impl MdApp {
    ...
    pub(crate) fn do_open(&mut self) {
            ...
            .pick_file()
        {
            match Self::import_to_md(&path) {
                Ok(content) => {
                    // Assign stable ids to display equations for the edit-history
                    // log, keeping the editor buffer marker-free (re-injected on save).
                    let (marked, ids) = mdall_core::equation_history::ensure_ids(&content);
                    self.doc_eq_ids = ids;
                    self.source = mdall_core::equation_history::strip_ids(&marked);
    ...
    pub(crate) fn do_save(&mut self) {
        if let Some(ref path) = self.current_file.clone() {
            // Re-inject the equation-history id markers so identity survives reload.
            let to_write = mdall_core::equation_history::inject_ids(&self.source, &self.doc_eq_ids);
...
impl MdApp {
    ...
    /// Left-hand panel: the timestamped edit history of the current document's
    /// equations, with one-click revert (the "git for equations" surface).
    pub(crate) fn render_history_panel(&mut self, ctx: &egui::Context) {
        ...
        let path = match self.current_file.clone() {
            ...
            None => {
                egui::SidePanel::left("history_panel").default_width(300.0).show(ctx, |ui| {
                    ui.add_space(6.0);
                    ui.horizontal(|ui| {
                        ui.label(egui::RichText::new(t("panel.history")).heading());
                    ...
                    ui.label(egui::RichText::new("Open a saved document to see its equation history.")
                        .small().weak());
                });
                return;
            }
        };
        let hist = mdall_core::equation_history::history_for("MD-ALL", &path);
        let mut close = false;
        let mut revert_action: Option<(String, String)> = None;
        egui::SidePanel::left("history_panel")
            ...
            .show(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    ui.label(egui::RichText::new(t("panel.history")).heading());
                    ui.label(egui::RichText::new(format!("({})", hist.equations.len())).weak());
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                ui.label(egui::RichText::new("Every equation edit, timestamped. Click a version to revert.")
                    .small().weak());
                ui.separator();
                if hist.equations.is_empty() {
                    ui.label(egui::RichText::new("No equation changes recorded yet. Edit an equation and it appears here.")
                        .small().weak());
                }
                egui::ScrollArea::vertical().show(ui, |ui| {
                    for (i, eq) in hist.equations.iter().enumerate() {
                        egui::Frame::group(ui.style()).show(ui, |ui| {
                            ui.horizontal(|ui| {
                                ui.label(egui::RichText::new(format!("Eq {}", i + 1)).strong());
                                let n = eq.change_count();
                                if n > 0 {
                                    ui.label(egui::RichText::new(format!("{} modif{}", n, if n == 1 { "" } else { "s" }))
                                        .small().color(crate::theme::ACCENT));
                                }
                            });
                            let cur: String = eq.current().chars().take(64).collect();
                            ui.label(egui::RichText::new(cur).small().monospace());
                            egui::CollapsingHeader::new(egui::RichText::new("changes").small())
                                .id_salt(("eqhist", i))
                                .show(ui, |ui| {
                                    let orig: String = eq.origin.chars().take(48).collect();
                                    ui.horizontal(|ui| {
                                        ui.label(egui::RichText::new("origin").small().weak());
                                        if ui.small_button("\u{21A9}").on_hover_text("Revert to origin").clicked() {
                                            revert_action = Some((eq.id.clone(), "origin".to_string()));
                                        }
                                    });
                                    ui.label(egui::RichText::new(orig).small().monospace().weak());
                                    for c in &eq.changes {
                                        ui.horizontal(|ui| {
                                            ui.label(egui::RichText::new(&c.by).small().weak());
                                            ui.label(egui::RichText::new(c.ts.chars().take(19).collect::<String>())
                                                .small().weak());
                                            if ui.small_button("\u{21A9}").on_hover_text("Revert to this version").clicked() {
                                                revert_action = Some((eq.id.clone(), c.ts.clone()));
                                            }
                                        });
                                        let to: String = c.to.chars().take(48).collect();
                                        ui.label(egui::RichText::new(to).small().monospace());
                                    }
                                });
                        });
                        ui.add_space(4.0);
        ...
        if let Some((eq_id, ts)) = revert_action {
            if let Some(latex) = mdall_core::equation_history::revert("MD-ALL", &path, &eq_id, &ts, "user") {
                let marked = mdall_core::equation_history::inject_ids(&self.source, &self.doc_eq_ids);
                if let Some(new_marked) = mdall_core::equation_history::set_equation_latex(&marked, &eq_id, &latex) {
                    self.source = mdall_core::equation_history::strip_ids(&new_marked);
                    if let Some(slot) = self.doc_eq_ids.iter_mut().find(|(id, _)| id == &eq_id) {
                        slot.1 = latex.clone();
                    }
                    self.modified = true;
                    self.segments_dirty = true;
                    self.status_msg = "Equation reverted".into();
                }
            }
        }
    }
